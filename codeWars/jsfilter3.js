...
He has troubles with users ending or starting in a ".", and his user-array is a 
flat array of user-email-pairs, like so:
...
so no need to keep the user-name, only e-mail addresses for the user-names that 
start or end with a "." should be returned. For the above array, the correct 
return-array would be
...
function searchNames(logins){
  return logins.filter(function(val, index){
    return logins[index-1] && logins[index-1].match(/^\.|\.$/);
  });
}
